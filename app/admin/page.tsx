        ...
        {mainPages.map((page, pageIndex) => {
          ...
          return (
            <section className={`${styles.card} ${styles.menuTreeGroup}`} key={page.id}>
              <div className={styles.pageOrderControls} aria-label={`${page.title} sıralaması`}>
                <button type="button" disabled={pageIndex === 0} onClick={() => moveTopLevelPage(page.id, -1)} title="Yukarı taşı">↑</button>
                <span>{pageIndex + 1}</span>
                <button type="button" disabled={pageIndex === mainPages.length - 1} onClick={() => moveTopLevelPage(page.id, 1)} title="Aşağı taşı">↓</button>
              </div>
              ...
              {expandedPageId === page.id && (
                <div className={styles.pageAccordionContent}>
                  <div className={styles.menuTreeHeader}>
                    ...
                    <div className={styles.menuTreeActions}>{page.menuType === "direct" && <a href={managedPageHref(page)} target="_blank">Aç ↗</a>}{!page.locked && <button type="button" onClick={() => setPages((current) => current.filter((item) => item.id !== page.id && item.parentId !== page.id))}>Sil</button>}</div>
                  </div>
        ...
        <div className={styles.mobileMenuControls}>
          <section className={styles.card}>
            ...
            <div className={styles.headerForm}>
              ...
              <label className={styles.headerCheck}><input type="checkbox" checked={settings.mobileHeaderSticky} onChange={(event) => setSettings({ ...settings, mobileHeaderSticky: event.target.checked })} /> Mobilde header sayfayla birlikte sabit ilerlesin</label>
            </div>
          </section>
...
function LegacyModuleManager({ showToast }: { showToast: (message: string) => void }) {
  ...
  function toggleCategory(id: typeof donationCategoryOptions[number][0]) {
